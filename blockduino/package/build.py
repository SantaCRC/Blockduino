...
# -*- coding: utf-8 -*- #
#
# Builds the Blockduino Python portion of the app for Linux or OS X.
#
...
# build of the Blockduino application.
...
def create_shell_file(os_type):
    ...
    easily launch the Blockduino application.
    ...
    if os_type == "linux":
        shell_text = '#!/bin/bash\n' \
                     ...
                     './%s' % os.path.join(exec_folder_name, "Blockduino")
        shell_location = os.path.join(project_root_dir, "Blockduino_run.sh")
    elif os_type == "windows":
        shell_text = "@echo off\n" + \
                     "start %s" % os.path.join(exec_folder_name,
                                               "Blockduino.exe")
        shell_location = os.path.join(project_root_dir, "Blockduino_run.bat")
    else:
    ...
    try:
        ...
        print(script_tab + "ERROR: Shell file to launch the Blockduino "
...
def build_ardublockly():
    ...
    os_type = get_os()
    print(script_tag + "Building Blockduino for %s." % os_type)
    ...
    print(script_tag + "Removing old Blockduino executable directory.")
    ...
    print(script_tag + "Creating shell file to easily execute Blockduino.")
