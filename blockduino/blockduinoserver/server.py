# -*- coding: utf-8 -*-
"""Launch the Blockduino Server and handle all requests.
...
from blockduinoserver import actions
import webview
import serial
import subprocess
...
#
@app.route('/')
@app.route('/blockduino')
def index_redirect():
    """Redirect the server entry point to the Blockduino front end."""
    redirect('/blockduino/index.html')


@app.route('/blockduino/<file_path:path>')
def static_ardublockly(file_path):
    """Serve the 'Blockduino' folder static files.

    :param file_path: File path inside the 'Blockduino' folder.
    :return: Full HTTPResponse for the static file.
    """
    return static_file(file_path,
                       root=os.path.join(document_root, 'blockduino'))
...
def serialo():
    ...
    thread.start()
    window = webview.create_window('Monitor Serial', 'http://localhost:8000/blockduino/test.html')
...
def serialopen():
    ...
    if salir:
        ...
        the_other_process = subprocess.Popen(['python', 'blockduinoserver/console.py'])
    # global salir
